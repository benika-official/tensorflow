    ...
    for (auto it = queues_.rbegin(); it != queues_.rend(); ++it) {
      ...
      if (it->second && !it->second->empty()) {
        std::vector<std::unique_ptr<TaskType>> t =
            it->second->RemoveTask(remaining_size);
        if (t.empty() && enable_large_batch_splitting_) {
          uint64_t start_time = it->second->EarliestTaskStartTime().value();
          std::unique_ptr<TaskType> task = it->second->RemoveTask();
          std::vector<std::unique_ptr<TaskType>> output_tasks;
          ...
          for (size_t i = output_tasks.size() - 1; i >= 1; --i) {
            it->second->PrependTask(std::move(output_tasks[i]), start_time);
          }
        } else {
